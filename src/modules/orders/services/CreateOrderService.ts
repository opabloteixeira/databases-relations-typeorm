...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    const customersExists = await this.customersRepository.findById(customer_id);

    if(!customersExists){
      throw new AppError('Could not find any customer with the given is')
    }
    ...
    const existentProducsIds = existentProducts.map(product => product.id)

    const checkInexistentProducts = products.filter(
      product => !existentProducsIds.includes(product.id)
    )
